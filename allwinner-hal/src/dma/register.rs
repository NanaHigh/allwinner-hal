...
impl IrqEnable1 {
    ...
    #[inline]
    const fn get_bit_offset(channel: u8, interrupt_type: InterruptType) -> u32 {
        assert!(channel >= 8 && channel <= 15, "Channel must be 8-15 for IrqEnable1");
...
impl AutoGating {
    ...
    #[inline]
    pub const fn from_raw(value: u32) -> Self { 
        Self(value & 0x7) // Only bits 0-2 are valid
    }
...
impl Status {
    ...
    /// Creates a new `Status` register value from a raw u32 value.
    #[inline]
    pub const fn from_raw(value: u32) -> Self {
        Self(value)
    }

    /// Returns the raw u32 value of the register.
    #[inline]
    pub const fn raw(self) -> u32 {
        self.0
    }
    ...
    pub const fn is_dma_channel_idle(self, channel: u8) -> bool {
        assert!(channel < 16, "Channel number must be 0-15");
        (self.0 & (1 << channel)) == 0  // 直接检查，不调用其他函数
    }
...
mod tests {
    ...
    fn test_irq_pending0() {
        let reg = IrqPending0::from_raw(0x0);
        ...
        assert_eq!(reg.raw(), 0x21);
        
        // Test reading existing pending
        let reg = IrqPending0::from_raw(0x4);
    ...
    fn test_irq_pending1() {
        let reg = IrqPending1::from_raw(0x0);
        
        let reg = reg.clear_irq(8, InterruptType::HalfPackage);
        assert!(reg.if_irq_pending(8, InterruptType::HalfPackage));
        assert_eq!(reg.raw(), 0x1);
        
        let reg = reg.clear_irq(14, InterruptType::QueueEnd);
        assert!(reg.if_irq_pending(14, InterruptType::QueueEnd));
        assert_eq!(reg.raw(), 0x04000001);
        
        // Test reading existing pending
        let reg = IrqPending1::from_raw(0x4);
        assert!(reg.if_irq_pending(8, InterruptType::QueueEnd));
        assert!(!reg.if_irq_pending(8, InterruptType::HalfPackage));
    }

    #[test]
    fn test_auto_gating() {
        let reg = AutoGating::from_raw(0x0);
        ...
        assert_eq!(reg.raw(), 0x6);
        
        // Test init recommended
        let reg = AutoGating::init_recommended();
        assert_eq!(reg.raw(), 0x4);
        assert!(!reg.is_dma_mclk_circuit_gating_enabled());
        assert!(reg.is_dma_common_circuit_gating_enabled());
        assert!(reg.is_dma_chan_circuit_gating_enabled());
        
        // Test bit mask
        let reg = AutoGating::from_raw(0xFFFFFFFF);
        assert_eq!(reg.raw(), 0x7);
    }

    #[test]
    fn test_status() {
        let reg = Status::from_raw(0x8000FFFF);
        assert_eq!(reg.raw(), 0x8000FFFF);
        assert!(!reg.is_mbus_fifo_empty());
    ...
    fn test_irq_pending1_invalid_channel() {
        let reg = IrqPending1::from_raw(0x0);
        reg.if_irq_pending(75, InterruptType::HalfPackage);
    }
